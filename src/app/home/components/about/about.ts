...
import { iAboutModel } from '../../../shared/models/about';
...
export class About implements OnInit, OnChanges {
  @Input() userId!: number;

  // les données par défaut (fallback)
  componentId = Math.random().toString(36).substring(7); // Identifiant unique


  private readonly DEFAULT_ABOUT: iAboutModel = {
    greeting: 'Hi there!',
    intro: `
      I am 20 years old and a third-year Software Engineering student.
      I enjoy developing applications, but solving real problems and helping people is even better!
      As a versatile developer, I combine full-stack and UI/UX skills to create useful and engaging solutions.
    `,
    skills: [
      { label: 'Languages & Frameworks', value: 'Flutter, Python, Java, Django' },
      { label: 'Web & Front-end', value: 'HTML, CSS, UI / UX' },
      { label: 'Databases', value: 'SQL, MySQL' },
      { label: 'Tools & Software', value: 'GitHub, VS Code, Visual Studio, Arduino IDE, PowerPoint, Canva' },
      { label: 'Languages', value: 'French (native), English (intermediate)' }
    ],
    contacts: [
      { label: 'Email', value: 'ornella05koffi@gmail.com' },
  ...
  about: iAboutModel = { ...this.DEFAULT_ABOUT }; // Copie des données par défaut
  ...
  // Méthode pour sécuriser les SVG
  sanitizeSvg(svg: string): SafeHtml {
    return this.sanitizer.bypassSecurityTrustHtml(svg);
  }

  ngOnInit() {
    console.log(`🔵 COMPOSANT ${this.componentId} - ngOnInit`);
    const userId = this.route.snapshot.paramMap.get('id');
    console.log(`🔵 COMPOSANT ${this.componentId} - userId:`, userId);

    if (userId) {
      this.userService.getUser(+userId).subscribe({
        next: (user) => {
          const newAbout = this.mapUserToAbout(user);
          this.about = newAbout;
          this.cdr.markForCheck(); //verifie que les données sont mises à jour et déclenche la détection de changement
        }
      });
    }
  }

  // Méthode pour détecter les changements de l'ID utilisateur
  ngOnChanges(changes: SimpleChanges) {
    if (changes['userId'] && !changes['userId'].isFirstChange()) {
      const newUserId = changes['userId'].currentValue;
      if (newUserId) {
        this.userService.getUser(+newUserId).subscribe({
          next: (user) => {
            this.about = this.mapUserToAbout(user);
            this.cdr.markForCheck(); // Pour forcer la détection
          },
  ...
  //Méthode pour mapper les données de l'utilisateur à notre modèle iAboutModel
  private mapUserToAbout(user: any): iAboutModel {  // Retourne un nouvel objet
    console.log('User reçu:', user);
    console.log('Skills reçus:', user.skills_fk);
    console.log('Socials reçus:', user.sociallinks);

    const hasSkills = user.skills_fk && user.skills_fk.length > 0;
    const hasSocials = user.sociallinks && user.sociallinks.length > 0;

    console.log('Has skills:', hasSkills);
    console.log('Has socials:', hasSocials);

    // Retourne un NOUVEL OBJET
    return {
      greeting: `Hi! I'm ${user.prenom || ''} ${user.nom || ''}`.trim(),
      intro: user.description || this.DEFAULT_ABOUT.intro,
      image: user.photo_profil || this.DEFAULT_ABOUT.image,
      cvUrl: user.lien_cv || '#',

      skills: hasSkills
        ? user.skills_fk.map((s: any) => ({
          label: s.label,
          value: s.value
        }))
        : [...this.DEFAULT_ABOUT.skills], // Copie des skills par défaut

      contacts: [
        ...(user.email ? [{ label: 'Email', value: user.email }] : []),
        ...(user.telephone ? [{ label: 'Téléphone', value: user.telephone }] : [])
      ],

      socials: hasSocials
        ? user.sociallinks.map((s: any) => ({
          icon: this.getSocialIcon(s.nom),
          url: s.lien
        }))
        : [...this.DEFAULT_ABOUT.socials], // Copie des socials par défaut

      technologies: [...this.DEFAULT_ABOUT.technologies] // Copie des technologies par défaut
    };
  }

  // Méthode pour obtenir l'icône sociale en fonction du nom de la plateforme
  private getSocialIcon(platform: string): string {
    console.log('Platform reçue:', platform);
    ...
    const icon = icons[platform?.toLowerCase()] || 'fab fa-link';
    console.log('Icon choisie:', icon);
    return icon;
  }
}